    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            if (leafContainerRef.current) {
                const leaves = leafContainerRef.current.leaves;
                if (leaves) {
                    leaves.width = leafContainerRef.current.offsetWidth;
                    leaves.height = leafContainerRef.current.offsetHeight;
                    // You could also call a method on leaves to reinitialize or update the scene as needed
                }
            }
        };
        ...
        if (leafContainerRef.current) {
            ...
            leafContainerRef.current.leaves = leaves; // Store the scene instance for later use
            leaves.init();
            leaves.render();
        }
